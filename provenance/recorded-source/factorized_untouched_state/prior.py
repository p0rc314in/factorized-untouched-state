"""A host-side factorization of SDM's learned initial memory parameter."""
...
def initialize_full_memory_from_product(
    mixer: nn.Module,
    *,
    full_table_std: float,
    materialization_dtype: torch.dtype | None = None,
) -> None:
    """Initialize a dense table from the exact product-factor draw.

    The temporary factors consume the same role-keyed random draws and use the
    same materialization operation as ``AdditiveProductMemory.forward``. The
    resulting dense parameter is therefore functionally identical to the tied
    product representation before the first optimizer step.
    """

    memory = getattr(mixer, "memory", None)
    if not isinstance(memory, nn.Parameter):
        raise ValueError("materialized product initialization requires learned memory")
    if parametrize.is_parametrized(mixer, "memory"):
        raise ValueError("materialized product initialization requires a dense memory")
    heads = int(mixer.num_heads)
    slots_per_head = int(mixer.slots_per_head)
    codebook_size = math.isqrt(slots_per_head)
    if codebook_size * codebook_size != slots_per_head:
    ...
    with torch.no_grad():
        ...
        materialized_row = (
            row if materialization_dtype is None else row.to(materialization_dtype)
        )
        materialized_column = (
            column
            if materialization_dtype is None
            else column.to(materialization_dtype)
        )
        memory.copy_(
            (
                materialized_row[:, :, None, :]
                + materialized_column[:, None, :, :]
            )
            .reshape_as(memory)
            .to(memory.dtype)
        )
...
__all__ = [
    ...
    "initialize_full_memory_from_product",
