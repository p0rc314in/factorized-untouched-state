#!/usr/bin/env python3
"""Train one corrected SDM-family arm on canonical WikiText-103 coverage."""
...
PROJECT_MODEL_IDENTITY = json.loads((ROOT / "PROJECT_IDENTITY.json").read_text())[
    "model_identity"
]
LAYOUT = "BBBBBBBA"
WIDTH = 128
...
def verified_dependency_environment() -> dict[str, Any]:
    ...
    sdm_root = Path(os.environ["SDM_SOURCE_ROOT"]).resolve()
    sdm_file = Path(inspect.getfile(SparseDeltaMemory)).resolve()
    if sdm_root not in sdm_file.parents:
        raise RuntimeError(f"SDM resolved outside accepted overlay: {sdm_file}")
    return {
        "sdm_source_file": str(sdm_file),
        "sdm_source_root": str(sdm_root),
        "sdm_source_revision": os.environ["SDM_SOURCE_REVISION"],
        "sdm_loader_patch_sha256": os.environ["SDM_LOADER_PATCH_SHA256"],
...
def validate_resolved_config(args: argparse.Namespace, data: CanonicalWikiTextData) -> TrainConfig:
    ...
    if args.seed != 0:
        raise ValueError("this explicitly non-release-bearing campaign is seed 0 only")
    if args.release_scope != "single_seed_non_release_bearing":
        raise ValueError("single-seed campaign must remain non-release-bearing")
    expected = {
        "layout": LAYOUT,
        "width": WIDTH,
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--release-scope",
        choices=("single_seed_non_release_bearing",),
        required=True,
    )
