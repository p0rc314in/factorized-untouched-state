...
def load_recovery(
    ...
    *,
    ...
    if not isinstance(mechanism_diagnostics, list):
        raise RecoveryError("recovery checkpoint has invalid mechanism diagnostics")
    diagnostic_steps = _ordered_steps(
        mechanism_diagnostics,
        name="mechanism_diagnostics",
    )
    if diagnostic_steps and diagnostic_steps[-1] > step:
        raise RecoveryError("mechanism diagnostics extend beyond the recovery step")
