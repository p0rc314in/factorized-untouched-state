...
from factorized_untouched_state.prior import (
    INITIAL_STATE_MODES,
...
def resolve_prior_initialization(
    ...
    resolved = (
        ...
        if prior_initialization is None
        else prior_initialization
    )
    allowed = {
        "zero": {"none"},
        "product": {"product_factors"},
        "full": {"product_materialized", "independent_full"},
    }
    if resolved not in PRIOR_INITIALIZATIONS or resolved not in allowed[initial_state]:
        raise ValueError(
    ...
    def __init__(
        ...
        *,
        ...
        if mechanism_diagnostics:
            mixer_args["log_memory_access_stats"] = True
        self.mixer = SparseDeltaMemory(
            SparseDeltaMemoryArgs(**mixer_args),
            layer_id=layer_idx,
        )
...
class MatchedLanguageModel(nn.Module):
    ...
    def __init__(
        ...
        *,
        ...
        mechanism_diagnostics: bool = False,
    ) -> None:
        super().__init__()
        if initial_state not in INITIAL_STATE_MODES:
        ...
        self.mechanism_diagnostics = mechanism_diagnostics
        self.layout = layout
        self.width = width
        self.heads = heads
        ...
        for layer_idx, (kind, layer) in enumerate(zip(self.layout, self.layers)):
            ...
            if self.initial_state in ("product", "full"):
                with torch.random.fork_rng(devices=rng_devices):
                    ...
                    if self.prior_initialization == "product_materialized":
                        initialize_full_memory_from_product(
                            layer.mixer,
                            full_table_std=self.width**-0.5,
                            materialization_dtype=self.activation_dtype,
                        )
                    else:
                        initializer = {
                            "product_factors": initialize_product_factors,
                            "independent_full": initialize_full_memory,
                        }[self.prior_initialization]
    ...
    def arm_identity(self) -> dict[str, Any]:
        ...
        return {
            ...
            "mechanism_diagnostics": self.mechanism_diagnostics,
            "prior_initialization": {
